...
const LINK_HREFS = [
  { key: 'whatsapp', href: '', external: true },
...
export default function Contact() {
  ...
  const LINKS = LINK_HREFS.map(l => ({
    key: l.key,
    label: t(`links.${l.key}`),
    href: l.key === 'whatsapp' ? WA_HREF : l.href,
    external: l.external,
  }))
  return (
...
const LINK_CONFIG = {
  whatsapp: {
    ...
    shadow: '0 4px 24px rgba(37, 211, 102, 0.25)',
    icon: (
  ...
  linkedin: {
    ...
    hoverBorder: 'rgba(10, 102, 194, 0.7)',
    shadow: '0 4px 24px rgba(10, 102, 194, 0.25)',
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden style={{ flexShrink: 0 }}>
  ...
  email: {
    ...
    hoverBg: 'rgba(234, 67, 53, 0.2)',
    hoverBorder: 'rgba(234, 67, 53, 0.7)',
    shadow: '0 4px 24px rgba(234, 67, 53, 0.25)',
    icon: (
...
function ContactLink({ link }: { link: { key: 'whatsapp' | 'linkedin' | 'email'; label: string; href: string; external: boolean } }) {
  ...
  return (
    <a
      ...
      style={{
        ...
        border: `1px solid ${hov ? config.hoverBorder : config.border}`,
        boxShadow: hov ? config.shadow : 'none',
    ...
    >
      ...
      {link.external && (
        <span style={{ opacity: hov ? 1 : 0.5, transform: hov ? 'translate(2px,-2px)' : 'none', transition: 'opacity 0.2s, transform 0.2s', fontSize: '0.75rem' }}>↗</span>
      )}
    </a>
  )
}
